...
const RENEW_AFTER_MS = 20 * 60 * 60 * 1000; // renew after 20h, before the 24h expiry

async function ensureFreshToken() {
  if (Date.now() - tokenState.lastRenewed < RENEW_AFTER_MS) return;
  try {
    ...
    // common possibilities and log the raw response so this is easy to
    // verify/adjust the first time it runs.
    ...
    if (r.ok && newToken) {
      ...
      console.log('Dhan token renewal did not return a usable token — raw response:', JSON.stringify(data));
    }
  } catch (e) {
